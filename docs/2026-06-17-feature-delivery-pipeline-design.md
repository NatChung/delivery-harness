...
- **mock 生命週期 / superpowers 接縫**:mock data 在 Phase 3 隨 BDD 寫(測試先綠 = 驗證測試接上 prototype UI)→ **Phase 3 出場抽 mock 立 RED**;Phase 5 收到 RED 測試,由 `subagent-driven-development`+TDD 驅動 red→green。**implement 是 superpowers 領地,pipeline 不塞抽-mock 等自訂步驟** —— 只餵 RED、收 tests-green。
