...
> 把「客戶功能需求」從進場到上 stg 的整條流程,做成**可重複、狀態耐久、人在迴圈**的 pipeline。沿用 superpowers skills(`brainstorming → design.md` 慣例),補上 prototype-first + AC/BDD + TestID 幾個自訂階段。
>
> 日期:2026-06-17 ｜ rev 3(rev 2:design review 加失敗/rework 邊、branch model、mock-data 閘、spike track、並行/id、AC+TestID 慣例。rev 3 @2026-06-25:Phase 1 brainstorming 改產 `design.md` 指導 prototype;`spec.md` 退回 prototype 後的 AC/BDD+mock 驗收層;明訂 mock→RED 在 spec 出場、implement 維持純 superpowers)｜ 狀態:design(待 review → writing-plans)

## 目標 / 非目標

**目標**
- 一條可重複的 feature pipeline:需求 → brainstorming 產 `design.md` → UI prototype(依 design.md,真 app/web)→ 客戶定案 → spec(AC/BDD+TestID+mock;出場抽 mock 立 RED)→ plan(TDD)→ 開發 → 測試 pass → stg UAT。
...
- pipeline 名 = **feature**;入口 `/feature`;ticket 目錄 `docs/features/<NNN>-<slug>/`(ticket.md + design.md + spec.md + plan.md 同住)。
...
| **lite** | 無新 UI / 純後端 / 純修則 | 跳 Phase 2,brainstorming → design.md → spec → plan → TDD → UAT(無 prototype,design.md 直接接 spec)|
...
- 邏輯 / 計算 → **function/unit 測試**(`<mobile unit test>` / `<api test runner>` / `<web test stack>`)
- 後端行為 / 合約 → **API 測試**(`<api test runner>` e2e / curl 腳本)
- UI 行為 / 互動 → **Auto UI script**(`<mobile integration test>` / Playwright;重用 feature pipeline 的 TestID 慣例)
...
**bug 增量階段不依賴 intake skill(I4)**:bug track(分解 step 2)時 `feature-intake` 尚未建 → 直接 `cli.py new --track bug <slug>` 建 ticket。且該階段只有**後端/邏輯 bug** 可全自動(test 基建已在);**UI bug 的 Auto-UI 重現測試** 要等分解 step 4 的 cms/app integration bootstrap。
...
| 1 requirements | `brainstorming`:讀相關 code、問澄清題 → 產 **`design.md`**(UI/流程設計,指導 prototype;**不含** AC/BDD)| `design.md` 經 review 確認(入 artifacts)| 需求不清 → 停 `on-hold` 等人 | `brainstorming`(重用)|
...
| 3 spec | 讀 branch diff 當**起始集** + 讀懂 UI 樹 → UI 面 inventory → 注 TestID → 寫 AC/BDD + **mock data** → 產 `spec.md`;**測試靠 mock 先綠 → 出場抽 mock 立 RED** | `spec-red`(AC/BDD 測試紅、交給 implement)| — | `feature-spec`(新)|
| 4 plan | `spec.md` 餵 writing-plans → `plan.md`(TDD 排序)| plan.md | — | `writing-plans`(重用)|
| 5 implement | 接 spec 交來的 **RED 測試** → red-green;UI 骨架下補真邏輯(UI 保留不重做)。**純 superpowers 領地,不塞抽-mock 等自訂步驟** | unit/TDD+AC/BDD 全 pass **且 mock-data 清零 gate 過** | 測試紅 → 留在 Phase 5 | `subagent-driven-development`+`test-driven-development`+`executing-plans`(重用)|
...
- **design.md(Phase 1 brainstorming 產)指導 prototype;spec.md(Phase 3,prototype 後)= AC/BDD+TestID+mock 驗收層**。兩份各司其職(intent→acceptance);沿用 superpowers 的 `brainstorming → design.md` 慣例,不再用 spec.md 取代 design.md 槽位。
- **mock 生命週期 / superpowers 接縫**:**測試 fixture mock** 在 Phase 3 隨 BDD 寫(測試先綠 = wiring check)→ **Phase 3 出場抽掉立 RED**(gate `spec-red`);Phase 5 收到 RED 測試,由 `subagent-driven-development`+TDD 驅動 red→green。**implement 是 superpowers 領地,pipeline 不塞抽-mock 等自訂步驟** —— 只餵 RED、收 tests-green。(注:這條 mock ≠ Phase 2 的 prototype UI mock,後者由 implement 補真邏輯時替換、`mock-data-cleared` gate 驗清;見「Mock-data 清零 gate」段。)
...
- **只在 Phase 5 gate 過(測試 pass + mock-data 清零)才 merge 回真 branch** → 再上 staging。
- ticket `branch:` 記實際 branch;跨 repo 記多條。

## Mock-data 清零 gate(Phase 5→6,review Critical 3)

> **兩種 mock 別混(關鍵)**:
> 1. **測試 fixture mock**(Phase 3 隨 BDD 寫,使測試先綠 = wiring check)→ **Phase 3 出場抽掉 → 測試 RED**(gate `spec-red`),交給 implement。這是「抽 mock」那一步,發生在 **spec 出場、不在 implement 內**。
> 2. **prototype UI mock**(Phase 2 種在 app code 的 `// PROTOTYPE-MOCK` 假 data provider)→ Phase 5 implement 的 **TDD 補真邏輯時自然替換掉**,**本 gate(`mock-data-cleared`)只驗證它全清**。
>
> 所以本 gate 清的是 (2),**不是** implement 內的自訂抽-mock 步驟 —— implement 仍**純 superpowers**(red→green),清 (2) 是 red→green 的自然產物 + reviewer 把關,不違反「implement 不塞自訂步驟」。

UI 刻意保留 → 最大風險是假 data survive 到 prod。Phase 5→6 強制 gate:
- 該 feature code path 內**無 mock/stub data provider**(grep 約定標記 `// PROTOTYPE-MOCK`,須全清;與 integration-bundle convention 的 mock gate 同一標記)。
...
gates:                  # 單一真相:所有 signoff/gate 只記這裡
  ...
  - {name: design-review, status: pending, at: null}     # Phase 1 出場:design.md 經 review
  - {name: prototype-signoff, status: pending, at: null, by: null}
  - {name: spec-red, status: pending, at: null}          # Phase 3 出場:抽 mock、AC/BDD 測試 RED
...
artifacts: {design: null, spec: null, plan: null}
...
### `design.md` 內容契約(Phase 1 brainstorming 產,指導 prototype)
要解的問題 + 方案/UI 與流程設計(畫面清單、入口、主流程、狀態)+ 相關既有 code 摸查(codegraph)+ 待確認假設。**指導 Phase 2 prototype 怎麼做**;經 review 確認後才 advance。**不含** AC/BDD/TestID(那是 `spec.md`,prototype 定案後才寫)。

### `spec.md` 內容契約(Phase 3 產,餵 write-plan)
確認後需求 + 新 UI 面 inventory(diff 起始集 + 讀樹補全)+ **AC/BDD(Gherkin)** + **TestID map**(元件 TestID → 對應 AC scenario)+ **mock data(供測試先綠)** + branch ref + (spike 時)feasibility note。**spec 出場前抽 mock → 測試 RED**,即交給 Phase 5 的可執行驗收契約(implement 不再碰 mock)。
...
- **TestID 命名**:`<mobile framework>` `Key('feat-<NNN>-<area>-<element>')`;web `data-testid="feat-<NNN>-<area>-<element>"`。feature-scoped、穩定。
- **測試框架(per 平台)**:`<mobile framework>` widget + `<mobile integration test>`;cms(`<web framework>`)→ **`<web test stack>` + Playwright e2e**(選型參考各 repo CLAUDE.md);api `<api test runner>`;chat `<web test stack>`(+e2e)。
...
1. **測試基建**(AC/BDD+TDD gate 需要):
   - **cms(`<web framework>`):零測試** → **獨立 sub-spec** bootstrap(`<web test stack>`+Playwright;選型參考 cms repo CLAUDE.md)。**block 功能 C**。
   - app:有 unit/widget+`<mobile unit test>`,**缺 `<mobile integration test>`** → 補,讓 TestID-based AC 跑得動。
   - api/chat:已有,OK。
