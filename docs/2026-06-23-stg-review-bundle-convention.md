...
```
部署 STG —— **注意 `<your STG deploy script>` 的 git 副作用**:它通常 bump `<version-manifest file>`、`git commit`、建 build tag、**並 push 當前分支 + tag**。確認你的 deploy script 行為後再跑。
```bash
# app:在 stg-review-* 上跑;每輪 build number +1(否則 tag/commit 被跳過 = 無聲 no-op)
<your STG deploy script> <ver> <build+1> staging
...
<your STG deploy script> <ver> <build+1> staging   # build number 再 +1
