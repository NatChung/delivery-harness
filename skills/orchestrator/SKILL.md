...
4. **subagent 回** → 主迴圈**正式 invoke** `superpowers:requesting-code-review`(由它派 reviewer 兄弟 subagent,**也是背景**)→ 收到回饋時**正式 invoke** `superpowers:receiving-code-review` → provisional-decide 或 `cli.py advance` → 派下一步。⚠️ **別用「general-purpose reviewer + 自己 paraphrase 的 review prompt」取代正式 invoke** —— 那會繞過 review skill 的原文紀律(實測易犯)。
...
⚠️ 先 invoke 該 phase 對應的「生產類」skill 再動手（這是強制,不是建議）：
- 5-implement / bug-fix → `superpowers:test-driven-development`（先寫會紅的測試,再寫到綠）
...
- 不要叫 superpowers:requesting-code-review 或任何 review step（review 是 orchestrator 主迴圈的事;但上面「生產類」skill 你要正式 invoke,不能只照文字描述行為）
- 不要叫 cli.py advance
